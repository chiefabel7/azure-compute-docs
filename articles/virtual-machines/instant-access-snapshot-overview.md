...
Azure Managed Disk Snapshots provide point-in-time backups of disks that can be used as backup during software upgrade, disaster recovery, or creating copies of your production disks for pre-prod environment or expansion. When creating snapshots from Azure Managed Disks, Azure automatically copies the data from the disk to the snapshot in the background. 

Snapshots from Premium SSD, Standard SSD, and Standard HDD disks are instantly accessible by default. Immediately upon creation, these snapshots can be used to restore new disks, download underlying data, and copy to other Azure regions. Disks restored from these snapshots are usable right away. Azure automatically initiates a background data copy from the snapshot to the disk, during which disk will experience a temporary performance drop until the copy completes.

Snapshots from Premium SSD v2 and Ultra disks require the background data copy to complete before they can be used. You can monitor the copy progress using the snapshot’s CompletionPercent property. To enable instant access for Premium SSD v2 and Ultra disk snapshots, you must explicitly set Instant Access property during snapshot creation. Snapshots created with instant access allow immediate disk restoration without waiting for the background copy to finish. Restored disks are rapidly hydrated with minimal performance impact. However, data download and cross-region copy are not available during the instant access period; these actions require the hydration process to complete.

Instant Access Snapshots from Premium SSDv2 and Ultra disk deliver high performance and rapid provisioning, making them ideal for scenarios where speed is critical:
...
## **Snapshots from Premium SSD, Standard SSD and Standard HDD**
Snapshots created from Premium SSD, Standard SSD, and Standard HDD instant available by default at creation. Snapshots can be used to create new disks, download underlying data via SAS URI, and copy to another Azure region for disaster recovery. No additional configuration or parameters are required.

Disks created from snapshots can be attached to running VMs instantly. At the same time, Azure initiates background data copy to hydrate the disk from snapshot data. During this phase, disk performance will be degraded until the copy completes. Snapshots are also available upon creation for creating new disks of different SKU. For example, to upgrade a Premium SSD to an Ultra disk, you can create a snapshot and restore it as an Ultra disk right away, no wait time required.

Upon creation, snapshots created from Premium SSD, Standard SSD, and Standard HDD disks support immediate SAS URI generation for secure data download to on-premises environments or other Azure storage accounts. Snapshots can also be copied to other Azure regions immediately upon creation for regional disaster recovery.

### Limitations
1.	For snapshots created from Premium SSD, Standard SSD, and Standard HDD, the CompletionPercent property always shows 100% at creation—even if the background data copy is still in progress.
2.	Premium SSD, Standard SSD, and Standard HDD disks created from snapshots show CompletionPercent as 100% immediately after creation, regardless of background data copy status for disk hydration.
3.	Premium SSD, Standard SSD, and Standard HDD disks created from snapshots will experience temporary performance degradation until the background copy completes. To reduce latency, use Premium Full Snapshots.
4.	While snapshots from Premium SSD, Standard SSD, and Standard HDD disks are instantly available for disk creation, they do not support fast restore capabilities that accelerate hydration and minimize latency.


## **Instant Access Snapshots for Ultra and Premium SSD v2 (Preview)**
Instant access is a new snapshot attribute available when creating snapshots from Premium SSD v2 and Ultra Disks. Once specified, the snapshot remains in instant access state for the duration defined by InstantAccessDurationMins. You can use snapshot in instant access state to create disk immediately  - no need to wait for the snapshot background data copy to complete. Newly restored disks are rapidly hydrated with minimal performance impact.

After specified instant access duration, it becomes to a Standard Storage snapshot and remains usable. You can monitor its state using the SnapshotAccessState property. Until the data is fully copied to Standard Storage, snapshot in instant access state rely on the availability of the source disk and do not protect against zonal outages. To protect against zonal failures, wait until CompletionPercent reaches 100%. For regional protection, copy the snapshot to another region.
...
### Create Instant Access Snapshot from Premium SSD v2 and Ultra Disk
...
1.	Pending: The snapshot is not usable for restore, copy, or download.
    a.	Applies to incremental snapshots from Premium SSD v2 and Ultra disks while background data copy is in progress.
    b.	Also applies to snapshots being copied across regions using CopyStart, until the copy completes.
2.	Available: The snapshot is available for restore (with perf impact), copy, or download.
    a.	Applies to snapshots from Premium SSD v1, Standard SSD, and Standard HDD.
    b.	Also applies to incremental snapshots from Premium SSD v2 and Ultra disks once background data copy completes.
    c.	Snapshots copied within the same region using Copy (shallow copy) also fall into this state.
3.	InstantAccess: The snapshot is usable for high-performance disk restore, but cannot be copied or downloaded.
    a.	Applies to incremental snapshots from Premium SSD v2 and Ultra disks where InstantAccessDurationMinutes has not expired, and snapshot background data copy is still in progress.
4.	AvailableWithInstantAccess: The snapshot is fully usable for restore, copy, and download, with high-performance restore still available.
    a.	Applies to incremental snapshots from Premium SSD v2 and Ultra disks where InstantAccessDurationMinutes has not expired, and background data copy has completed.
